...
@if(isset($cicloCerrado) && $cicloCerrado)
<div class="alert alert-warning">
    ...
    <span><strong>Ciclo Cerrado:</strong> No se pueden realizar modificaciones en productos de un ciclo cerrado.</span>
</div>
@endif
...
<div class="filters-section">
    ...
    <div class="filter-card">
        ...
        <select class="filter-select" id="lineaFilter" onchange="filterProducts()">
            <option value="">Todas las líneas</option>
            @foreach($lineas as $l)
                <option value="{{ $l->idLinea }}" {{ request('linea') == $l->idLinea ? 'selected' : '' }}>
                    {{ $l->linea }}
                </option>
            @endforeach
        </select>
                    ...
                    <tr data-product-id="{{ $p->idProducto }}" data-marca="{{ $p->marcaMkt->marca->marca ?? 'N/A' }}" data-core="{{ $p->core->core ?? 'N/A' }}">
                        ...
                        <td class="text-right">
                            <div class="action-buttons">
                                @php
                                    $productoEsCerrado = false;
                                    if ($p->ciclo) {
                                        if ($p->ciclo->fechaFin) {
                                            $fechaFin = \Carbon\Carbon::parse($p->ciclo->fechaFin)->startOfDay();
                                            $hoy = \Carbon\Carbon::now()->startOfDay();
                                            $productoEsCerrado = $fechaFin->lt($hoy);
                                        }
                                        if (!$productoEsCerrado && $p->ciclo->relationLoaded('estado') && $p->ciclo->getRelation('estado')) {
                                            $estadoRelacion = $p->ciclo->getRelation('estado');
                                            if ($estadoRelacion && $estadoRelacion->estado === 'Cerrado') {
                                                $productoEsCerrado = true;
                                            }
                                        }
                                    }
                                @endphp
                                <button class="action-btn action-edit" 
                                        ...
                                        title="{{ $productoEsCerrado ? 'No se puede editar (ciclo cerrado)' : 'Editar' }}"
                                        {{ $productoEsCerrado ? 'disabled' : '' }}>
